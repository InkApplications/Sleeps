...
class JvmSunScheduleProviderTest {
    ...
    fun beforeSunrise() {
        ...
        val provider = JvmSunScheduleProvider(object: Clock {
        ...
        assertEquals(6, schedule.dayOfMonth)
        assertEquals(2024, schedule.year)
    }

    @Test
    fun afterSunrise() {
        val now = LocalDate(2024, 1, 6).atTime(LocalTime(8, 2, 3)).atZone(TimeZone.of("America/New_York"))
        val provider = JvmSunScheduleProvider(object: Clock {
            override fun now(): Instant = now.instant
        }.atZone(now.zone))
